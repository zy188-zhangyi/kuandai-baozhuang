<template>
	<view class="">
		<view class="u-flex u-col-bottom box-sift u-p-l-20 u-p-r-20">
			<view class="u-flex-1 u-m-r-20 u-p-20" @click="clickType">
				<view class="color-white u-font-sm">
					<text class="u-m-r-15">{{typeValue}}</text>
					<u-icon name="arrow-down" color="#ffffff" size="28"></u-icon>
				</view>
				<template v-if="dateList">
					<u-select :default-value="typeIndex" v-model="typeShow" mode="single-column" :list="typeList"
						@confirm="typeConfirm"></u-select>
				</template>
			</view>

			<view class="u-box-w u-m-l-30 u-p-20" @click="clickDate">
				<view class="color-white u-font-sm">
					<text class="u-m-r-15">{{dateValue}}</text>
					<u-icon name="arrow-down" color="#ffffff" size="28"></u-icon>
				</view>
				<template>
					<u-select :default-value="dateIndex" v-model="dateShow" mode="mutil-column-auto" :list="dateList"
						@confirm="dateConfirm"></u-select>
				</template>
			</view>

			<view class="u-flex-1 u-m-l-30 u-p-20">
			...
			<view v-show="!screenShow" class="all-box-no" v-for="(item,sindex) in listInfo" :key="sindex">
				<view class="color-gray u-font-sm">
					...
					<view class="u-m-t-10 u-font-md">
						...
						<text v-show="item.orderType>=3" class="font-weight">----</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getOrders() {
				uni.request({
					url: this.$baseUrl + '/front/users/order', //仅为示例，并非真实接口地址。
					data: {
