<template>
	<view class="u-wrap page-box u-p-t-10 u-p-l-30 u-p-r-30">
		...
		<view class="box-map color-d">
			<p>1.如果您的定位不准确，请切换至您所在社区。</p>
			<p>2.如果找不到您的社区，请选择相应区域后，填写您的社区名称。</p>
				...
				uni.request({
					...
					success: (res) => {
						...
						if (result.code == 0) {
							...
							// 	if (this.areaId == item.id) {
							// 		this.lat1 = item.lat
							// 		this.lng1 = item.lng
							// 	}
							// 	let alls = parseFloat(item.lat) + ',' + parseFloat(item.lng)
							// 	if (index < 4){
							// 		all.push(alls)
							// 	}
								
							// })
							// let dta = this.getDistance(all)
							// console.log(dta)
							this.listInfo = list
						} else {
							this.showList = false
			...
			async getDistance(all) {
				if (all) {
					...
					uni.request({
						...
						data: {
							mode: 'walking',
							from: this.lat1 + ',' + this.lng1,
							to: out,
							key: this.$baseWxMapKey //获取key
						},
						success: (res) => {
							console.log(res);
							// if (res.data.result) {
